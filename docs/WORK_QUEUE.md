...
**Last updated:** 2026-08-26, round 9 (**RUNNER BUG FIXES + FOUR-TIER SOURCE HIERARCHY** — from Andy's live run 3: (1) runner bug fixed — OpenAI/Gemini empty-fingerprint issue traced to citation-noise + spelled-number gaps in the numeric-fingerprint regex, NOT Andy's Python 3.9/LibreSSL environment; re-verified against real run-3 model prose. (2) Citation checker now returns real diagnostics (http_status/content_length/content_type/word_overlap_ratio) instead of `error: None`, and a second bug found in the process — raw-HTML tag-concatenation was breaking substring matching against genuinely correct Cornell LII/eCFR sources, unrelated to source quality. (3) Four-tier source hierarchy (A/B/C/D) ratified and implemented: `source_tier`/`tier_rationale` added to the schema, backfilled corpus-wide (63 entries: A=12, B=6, C=43, D=2), full violation audit run — 27 of 37 nodes are sole-C/D-anchored, concentrated in AZ (7/7) and CA (7/7); federal spine fully clean. Detail in `docs/SOURCE_TIER_AUDIT_20260826.md`. (4) AZ Tier-A re-pin attempted, genuinely blocked — azleg.gov confirmed JS-gated, no Tier B fallback exists, Wayback Machine blocked at the tool level; flagged honestly, not silently downgraded. (5) CI validator bug fixed (schema check was sweeping up pipeline-output JSON under `rules/debt/validation/runs/`). (6) Disagreement queue purged of 72 auth-artifact entries from runs 1-2 (bad key formatting), replaced with a dated purge note per append-only discipline; 32 real entries remain. **NEXT run-command unblocked for Andy — see DAILY_CHANGELOG round 9 and the delivery message.** CA/UT/NY/remaining-TX re-pins diagnosed but not yet executed, queued below. Full detail: `docs/DAILY_CHANGELOG.md` round 9, `docs/SOURCE_TIER_AUDIT_20260826.md`.)

**Last updated:** 2026-08-26, round 8 (**PHASE A UNBLOCK + CONCEPT DEMO FIRST** — two directives executed same-day. ENG_HARDENING Task 1 (secret hygiene) CLOSED — security settings verified enabled, `.github/dependabot.yml` reconciled. Grounded-corroboration runner delivered (`scripts/corroboration/`), dry-run verified end-to-end. One new Band 3 node authored (`TX-DEFAULT-JUDGMENT-SET-ASIDE-DISCRETIONARY`) — corpus now 37 DRAFT nodes. Spec bumped to v5: census-audit numbers + CONCEPT-DEMO claim table (§8), concept-demo near-term target ahead of the ratified Stage 1/1.5/2 ladder (§10). `plugins/consumer-debt/skills/consumer-debt-validation/SKILL.md` reconciled to the current `rules/debt/` architecture — was a stale pre-existing skeleton, now wired up, v0.2.0-concept-demo. **The one Andy-dependency now on the critical path: run the corroboration package (`scripts/corroboration/README.md`) against the demo corpus (federal+TX+CA, ~18 nodes, ~$6 estimated) — this is what the 2-3 week concept-demo timeline hinges on.** NOW/HORIZON sections below updated accordingly. Full detail: spec Appendix 2 round 8, `docs/PROJECT_STATE_OF_RECORD.md`, `docs/DAILY_CHANGELOG.md`.)

**Last updated:** 2026-08-25, round 6 (**DEBT PHASE A ANCHOR-STATE BUILD COMPLETE** — AZ and NY built back-to-back per Andy's "do all the builds, one push at the end" instruction, batched into a single combined patch rather than three separate handoffs. AZ: 7 nodes, all `citation_verified: true`, notably a wage-garnishment formula more protective than the federal floor and a broad tools-of-trade exemption covering intangible business assets. NY: 6 nodes, all `citation_verified: true`; the homestead/vehicle/personal-property nodes required a second sourcing layer — NY's bare statutory text shows stale 2019 base dollar figures, so this round pulled and cited NY DFS's official triennial CPI-adjustment table alongside the statute to get the real current figures (effective 2024-04-01), the most rigorous sourcing exercise of any state layer so far. **Phase A's anchor-state build is now complete: 35 total DRAFT-tier grounded nodes across 8 files (federal spine 4 + TX 5 + CA 7 + UT 6 + AZ 7 + NY 6), all schema-validated and CI-passing.** Natural next phase is Phase B: running these nodes through actual multi-model grounded corroboration, which requires live API-key model access in Andy's environment. Nothing in this round rises to a genuine RED.)
...
| AZ state layer, 7 nodes (SOL written/oral, wage garnishment, homestead, vehicle, tools of trade, civil answer deadline) | ✅ DONE, DRAFT tier | `rules/debt/state/arizona/az_debt_state_layer_v1.json` — all 7 `citation_verified: true`; wage-garnishment formula notably more protective than federal floor |
| NY state layer, 6 nodes (contract SOL, income execution, homestead, vehicle, personal property, answer deadline) | ✅ DONE, DRAFT tier | `rules/debt/state/new_york/ny_debt_state_layer_v1.json` — all 6 `citation_verified: true`; homestead/vehicle/personal-property nodes cite both statutory base AND NY DFS's current triennial-adjusted dollar figures, since the bare statute alone understates real protection ~35-40% |
| **Phase A anchor-state build: COMPLETE** — federal spine (4 nodes) + all 5 locked states (TX/CA/UT/AZ/NY, 31 nodes) = 35 total DRAFT-tier nodes | ✅ DONE | See spec Appendix 2 for full roll-up |
| Band 3 discretionary node: `TX-DEFAULT-JUDGMENT-SET-ASIDE-DISCRETIONARY` (Craddock factors + TRCP 329b deadlines) | ✅ DONE, DRAFT tier | `rules/debt/state/texas/tx_debt_band3_discretionary_v1.json` — first non-Band-1 node in the corpus; corpus total now **37 DRAFT nodes** |
| Grounded-corroboration runner package (spec §3a/b/d) | ✅ DONE, awaiting Andy's live run | `scripts/corroboration/run_corroboration.py` + README/requirements/scenarios.json. Dry-run verified end-to-end this session. **Andy-dependency: live run against demo corpus is the critical-path hinge for the 2-3 week concept-demo target** |
| ENG_HARDENING Task 1 (secret hygiene) | ✅ CLOSED (2026-08-26) | Secret Protection, push protection, Dependabot alerts/security-updates confirmed enabled by Andy in GitHub UI; `.github/dependabot.yml` reconciled |
| Spec v5: §8 census-audit + CONCEPT-DEMO claim table; §10 concept-demo near-term target | ✅ DONE | See spec Appendix 2 round 8 |
| `plugins/consumer-debt/skills/consumer-debt-validation/SKILL.md` reconciled to `rules/debt/` | ✅ DONE, CONCEPT-DEMO status | Was a stale pre-2026-08-25 skeleton never wired to the current architecture — discovered and fixed this round |
...
| **AZ source-tier re-pin (all 7 AZ nodes)** | **azleg.gov confirmed JS-gated (direct fetch: "Javascript is required"); no Tier B fallback found (Cornell LII AZ page doesn't host statute text); Wayback Machine blocked at the tool level.** | A JS-capable fetch (Andy's own browser, or a future session with browser-automation tooling) pulling the rendered azleg.gov text for re-pinning as Tier A. Logged 2026-08-26 round 9, `docs/SOURCE_TIER_AUDIT_20260826.md`. |

---

## HORIZON (planned, not yet fully specified)

- **Source-tier re-pin queue (logged 2026-08-26 round 9).** 27 nodes are sole-C/D-anchored per the four-tier audit (`docs/SOURCE_TIER_AUDIT_20260826.md`) and not yet re-pinned: CA (7/7), UT (5/6), TX (4/6, excluding the already-clean homestead node), NY (3/6, excluding the already-clean DFS-anchored nodes). AZ is tracked separately above under BLOCKED (confirmed machine-hostile, different remediation path than the others). Recommended order for the next pass: UT and NY first (each already has a confirmed-working Tier A domain elsewhere in that state's own file — legacy.utcourts.gov, dfs.ny.gov — so the remaining gaps may be reachable via the same domain rather than a fresh source hunt), then CA, then the TX-specific gaps (one URL pattern that worked for the homestead node did not reproduce for three other TX chapters this round — returned nav-shell content instead of statute text — not yet diagnosed).

- **Debt track — post-concept-demo deferrals (logged 2026-08-26, "Concept Demo First" directive §1).** Not cancelled — each triggers on the concept demo succeeding (gate metrics ≥90%/≥90%, Andy's private run-through complete):
  - UT/AZ/NY full-layer reliance in demo scenarios (content already built, just not demo-relied-upon yet).
  - Formal Tier 1 narrative-perturbation sets and Tier 2 persona elicitation harness (spec §5) — this milestone uses the skill itself as the harness instead.
  - The standing adversarial red-team lane (spec §11d) — this milestone's adversarial pass runs per-node inside the corroboration runner, not as a standing lane.
  - Mutation-suite build-out (spec §3c) — flagged throughout this round's deliverables as not yet built; §4's full DRAFT→CORROBORATED promotion criterion technically needs it.
  - D-3 statute-and-case watch build (see `docs/DIRECTION_D_ROADMAP.md`).
  - Phase D statistical sampling audit / attorney certification (spec §3e-g) — the debt track's own release uses a census instead (spec §8, 2026-08-26 addition); Phase D's census-audit numbers (37-node corpus, 3.1-9.25 attorney-hours) are delivered and ready the moment Andy books review time, per spec §10's concept-demo critical-path table.
