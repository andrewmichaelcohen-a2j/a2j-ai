...
16. **[PROPOSED — GREEN once gate passes]** After the v3 dev regression passes: run the standing self-critique pass (Disciplines A/B/C) over the new `just_cause_attachment_threshold` element — specifically live-source verification of §1946.2(a)(2) verbatim text (the carried B4 flag from ratification). Produces `CA_NOTICE_SELF_CRITIQUE_REPORT_[date].md` addendum.
17. **[PROPOSED — RED-gated]** v0.4 golden-set candidate drafting (first batch under the amended Direction B protocol: full defect-class sweep per item, no model consultation during ground-truth review). Needs Andy's go/no-go + target defect mix; C-18-style attachment-threshold patterns are the natural regression-test candidates.
18. **[PROPOSED — GREEN]** MISSING_RULES_BACKLOG grooming: §1946.1(d) 30-day sale exception — draft only when Andy supplies/approves the escrow-condition statutory text (currently the sole backlog entry; no current item implicates it).
