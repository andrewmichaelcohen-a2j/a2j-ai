...
**Generated:** June 15, 2026 · **Last updated:** 2026-08-26, round 9 (**RUNNER BUG FIXES + FOUR-TIER SOURCE HIERARCHY.** From Andy's live run 3 with all three keys verified valid: (1) two runner bugs fixed in `run_corroboration.py`'s fingerprint-agreement logic — citation-reference noise and unnormalized spelled-out numbers were corrupting OpenAI/Gemini fingerprints where Anthropic's matched; re-verified against real run-3 prose (AZ-SOL-ORAL/-WRITTEN now correctly fingerprint to {'3'}/{'6'} across all three models). **Confirmed runner-side, not a Python 3.9/LibreSSL environment issue on Andy's machine** — directly answers his explicit question. (2) Citation checker now returns real diagnostics (http_status/content_length/content_type/word_overlap_ratio) instead of `error: None`; building this surfaced a second bug — raw-HTML tag concatenation (e.g. Cornell LII's `a<a>debt</a>collector`-style markup) was breaking substring matching against genuinely correct, reachable sources, meaning part of the run-3 "secondary sourcing" signal was actually a checker bug. (3) Four-tier source hierarchy (A/B/C/D) ratified by Andy and implemented: `source_tier`/`tier_rationale` added to `debt_schema_v1.0.json`, backfilled across all 9 rules files (63 source entries: A=12, B=6, C=43, D=2); full audit run — 27 of 37 nodes are sole-C/D-anchored (AZ 7/7, CA 7/7, UT 5/6, TX 4/6, NY 3/6), federal spine fully clean. Full distribution and node list in `docs/SOURCE_TIER_AUDIT_20260826.md`. (4) AZ Tier-A re-pin attempted per Andy's direction, genuinely blocked — azleg.gov confirmed JS-gated via direct fetch, no viable Tier B fallback found, Wayback Machine blocked at the tool level; reported honestly as an open gap rather than silently relabeled. CA/UT/NY/remaining-TX re-pins diagnosed, queued for the next pass (`docs/WORK_QUEUE.md` HORIZON). (5) CI validator bug fixed — schema check was sweeping up pipeline-output JSON under `rules/debt/validation/runs/`. (6) Disagreement queue purged of 72 auth-artifact entries from runs 1-2 (Andy's key-formatting errors), replaced with a dated purge note per append-only discipline; 32 real entries remain from runs 2-3. Full detail: `docs/DAILY_CHANGELOG.md` round 9.) · **Prior update:**

*(Preserved prior headline, 2026-08-26 round 8 -- moved here 2026-08-26 round 9 to make room for the new headline above without losing it, per the append-only documentation discipline:* **Last updated:** 2026-08-26 (**PHASE A UNBLOCK + CONCEPT DEMO FIRST.** Andy's two 2026-08-26 directives, executed same-day: (1) repo security settings verified and **ENG_HARDENING Task 1 (secret hygiene) CLOSED** -- Secret Protection + push protection + Dependabot alerts/security-updates all confirmed enabled, `.github/dependabot.yml` reconciled (matches Andy's spec exactly: github-actions ecosystem, `/` directory, weekly), logged as an owner-level exception to the one-writer pattern per Andy's own instruction; (2) grounded-corroboration runner package delivered -- `scripts/corroboration/run_corroboration.py` (+README/requirements/scenarios.json), implementing spec S3(a)/(b)/(d), dry-run verified end-to-end this session (37 nodes, all clean-pass); (3) one new Band 3 node authored (`TX-DEFAULT-JUDGMENT-SET-ASIDE-DISCRETIONARY`, the corpus's first genuinely-discretionary boundary marker); (4) `DEBT_PROJECT_ARCHITECTURE_SPEC.md` bumped to v5 -- S8 gains a census-audit subsection (37-node corpus, 3.1-9.25 attorney-hours estimate) and a CONCEPT-DEMO claim-language row, S10 gains a near-term concept-demo target (federal+TX+CA, CORROBORATED tier, 90%/90% machine-measured gate, 2-3 week critical path) ahead of the ratified Stage 1/1.5/2 ladder, which is retained as the next milestone, not deleted; (5) `plugins/consumer-debt/skills/consumer-debt-validation/SKILL.md` -- discovered this session as a stale pre-existing skeleton never wired to the current `rules/debt/` architecture, now reconciled and updated to v0.2.0-concept-demo. Corpus total: 37 DRAFT nodes (federal 5, TX 6, CA 7, UT 6, AZ 7, NY 6); UT/AZ/NY confirmed already fully built, exceeding the directive's "visible stub" ask, logged out-of-demo-reliance-scope. Full detail: spec Appendix 2 round 8, `docs/DAILY_CHANGELOG.md`.) · **Prior update:** *)*

*(Preserved prior headline, 2026-08-25 round 2 -- moved here 2026-08-26 to make room for the new headline above without losing it, per the append-only documentation discipline:* 2026-08-25, round 2 (**DEBT PHASE A BUILD STARTED.** Andy's build-authorization decision, in chat: v3 spec ratified as-is; ENG_HARDENING stays held with eviction except where applicable to debt as best practice — Tasks 2 (CI), 3 (schema — done), 4 (calibration suite), 7 (independent-review packaging) folded into Phase A per `DEBT_PROJECT_ARCHITECTURE_SPEC.md` §3; TX locked as the fifth anchor state (decided, not data-driven, logged as such); `A2J_STACK_AND_CJAC_SCOPE.md` confirmed final and promoted to README's first doc link; repo restructuring resolved — `rules/debt/` scaffolded fresh, live `rules/eviction/` left physically in place, 9 files' stale pre-relocation path fixed as a byproduct, stray `validation/l2/` and placeholder `.env`-pattern files removed. **Validation cadence for Phase A: build-first, AI-maximal, sampling-gated — human review does not gate the build-out** (Andy's words) — this is the ratified §3 sampling-audit model actually being run the way it was designed, not a redesign. First build output same-day: `rules/schema/debt_schema_v1.0.json`, `rules/debt/` scaffold, and one grounded node — FDCPA-VALIDATION-NOTICE-1692g (15 U.S.C. §1692g + 12 C.F.R. §1006.34, verbatim-cited, live-fetched 2026-08-25), honestly tagged DRAFT tier — single-model derivation, has not yet passed the multi-model corroboration/audit pipeline. Full detail: spec's new Appendix 2. Eviction line unaffected — still HOLD, keep-warm only, nothing in that line touched.)*)*
