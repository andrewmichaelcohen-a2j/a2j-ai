...
| Metric | Value | Notes |
|--------|-------|-------|
| Runs this cycle | 0 | No dispatcher fire; no Terminal/session runs since 07-20 10:31 PT |
| Standing dev set (07-20 gate) | 12/12 = 100%, α=1.000, newly_failing=0 | DUAL-MODEL-CONSENSUS, single_model_items=0; v3 fully active |
| Standing v0.3 held-out (BURNED) | 88.5% as-scored / 96.2% post-errata | Dual-reported; unchanged |
| Method rate / Overall rate (holdings) | N/A | No holdings run this cycle |
| Heartbeat classification (B-3) | `no-heartbeat` | Miss ×6 (07-16→07-21) |
| Cumulative (holdings) | MV=26, CI=4, RC=6 | Unchanged since 2026-07-06 |

**B1 (coverage):** no new score; standing figures unchanged (v0.3 held-out coverage 26/26 = 100%; dev 12/12 = 100%). **B2:** confident-wrong = 1 (C-18 class — v3 encodes the fix, dev-gate-passed; direct re-test of the C-18 pattern awaits v0.4). **B3:** no rule change this cycle; last check 07-20 gate — newly_failing=0. **B4 (currency):** carried flag unchanged — §1946.2(a)(2) additional-adult-tenant variant still not independently verified against verbatim statute text (refill proposal 16 executes this; gate condition it was waiting on has now passed).

**Process note (cadence):** report-side: sixth consecutive clean fire (07-16→07-21). Dispatcher-side: **MISSED FIRE ×6** — unchanged classification and unchanged single Andy action (plist reinstall + launchctl reload). With the regression gate already passed via Terminal, the reinstall's remaining payoffs are: (1) reopening the overnight lane, (2) giving D-1 its automatic noon driver (next eligible run ≥ 07-23 will otherwise silently not happen).

---

#### Morning report cycle — 2026-07-20 (**v0.3 HELD-OUT SCORED + BURNED, ERRATA APPLIED, §1946.2(a) RULE RATIFIED → v3 ACTIVE-PENDING-REGRESSION** — all session-driven since the 07-19 report, each logged in its own section; dispatcher: **FIFTH consecutive missed fire** [07-16→07-20], still `no-heartbeat`; report fired on time at ~8:00 AM)

*This cycle entry is a roll-up: none of the substantive movement came through the overnight dispatcher (which remains dark — `--heartbeat-status` → `{"state": "no-heartbeat"}`, `dispatcher_heartbeat.log` still does not exist, `launchd_stdout.log` last write still 2026-07-15 ~2:24 AM; fifth consecutive miss, agent-unloaded-leaning, the plist-reinstall/launchctl action remains the single convergent fix). All movement was Andy-session-driven on 07-19 → 07-20 morning and is already fully logged in the Broaden Proof 1 result writeup, the errata annotations, and the v3 version record in this file — summarized here for the cycle trend line. (1) **v0.3 held-out set scored ONCE and permanently BURNED (07-19):** 23/26 = 88.5% as-scored (95% CI [71.0%, 96.0%], Wilson), `consensus_status: DUAL-MODEL-CONSENSUS`, `single_model_items=0`, provenance verified (rules SHA = vProof1 `cc0cfab6…`; xlsx SHA = certified `e6dbb2fc…5df45`). (2) **Attorney Errata Memorandum same day (signed, `ERRATA_MEMO_v0_3_20260719.docx`):** frozen ground truth for C-21/C-22 was wrong (§1946.1(b)/Stancil governs independently of AB 1482 exemptions); post-errata **25/26 = 96.2%** (CI [81.1%, 99.3%]) — dual-reported everywhere per the memo, neither number superseded; new metric **ground-truth error rate 2/26 = 7.7%**. (3) **Miss autopsy → one genuine coverage gap (C-18, §1946.2(a) 12-month attachment threshold — rule genuinely absent) + one confirmed non-gap** (C-21/C-22 — (e)(7)/(e)(8) correctly do not reach `notice_period_too_short`; ratified as a wiring determination, not fixed). (4) **Rule proposal ratified by Andy → `ca_eviction_v3.json` cut 2026-07-20 ~07:32 PT** (SHA `65f1d9a4…947c7d`; vProof1 untouched, byte-frozen); `RULE_CHANGE_TRIGGER.flag` armed — **the next dev_set_monitor run IS the v3 regression gate** (12/12, newly_failing=0 required, else revert to vProof1). (5) **D-1 monitor did NOT run** — cadence-eligible since 07-19 but the dispatcher is dark and no Terminal run occurred (no new `dev_set_trend.jsonl` rows since the 07-16 baseline); the armed trigger now makes the next run (noon drain once the plist is reinstalled, or Terminal) do double duty as the regression gate. No new files in `l2/output/`, `results/`, `done/`, or `failed/` since run 9ae49b97 (ingested 07-09).*

| Metric | Value | Notes |
|--------|-------|-------|
| v0.3 held-out (one-shot, BURNED) | 23/26 = 88.5% as-scored · **25/26 = 96.2% post-errata** | Dual-reported per errata memo §4; Wilson CIs [71.0, 96.0] / [81.1, 99.3] |
| α (GPT vs Gemini, nominal, n=26) | **1.000** | Perfect model-pair agreement; DUAL-MODEL-CONSENSUS, single_model_items=0 |
| Ground-truth error rate | 2/26 = 7.7% | ERRATUM-2026-001 (C-21), -002 (C-22) — attorney-side, caught by the scoring loop |
| Method rate / Overall rate (holdings) | N/A | No holdings run this cycle |
| MV / CI / RC / PR / SM | — | No new buckets; PR quarantine unchanged (5 VT re-encounters from 9ae49b97) |
| Heartbeat classification (B-3) | `no-heartbeat` | Miss ×5 (07-16→07-20); launchd has still never invoked the instrumented dispatch.py |
| Cumulative (holdings) | MV=26, CI=4, RC=6 | Unchanged since 2026-07-06 |

**B1 (coverage):** v0.3 held-out — Coverage: 26/26 scored = 100% (no unknown-element omissions); Accuracy (known): 23/26 = 88.5% as-scored / 25/26 = 96.2% post-errata; Overall: same (coverage 100%). **B2:** confident-wrong = **3 as-scored → 1 post-errata** (C-18 only — the genuine coverage gap; v3 encodes the fix but it is NOT verified until the dev regression gate passes; C-21/C-22 reclassified — the models were right, the ground truth was wrong). **B3:** **regression check ✅ PASSED (2026-07-20 10:31 PT)** — 12/12, `newly_failing=0`, DUAL-MODEL-CONSENSUS; v3 is now fully active (see the v3 version record's gate row, updated same-day). **B4 (currency):** v3 embeds the 07-19 wiring determination in its own provenance; carried flag — the §1946.2(a)(2) additional-adult-tenant variant was ratified from attorney-supplied text but NOT independently verified against verbatim statute text by Cowork (flagged at proposal, flag survives ratification; recommend live-source confirmation next self-critique pass).

**Process note (cadence):** report-side: fired ~8:00 AM PDT — on time, **fifth consecutive clean fire (07-16→07-20)**. **Dispatcher-side: MISSED FIRE ×5** — unchanged classification (`no-heartbeat`), unchanged single convergent Andy action (plist reinstall + launchctl reload), which now carries a third payoff: the first noon drain after reinstall auto-runs the armed v3 regression gate inside the monitor's 09:00–23:00 window. Folded into the standing overnight-environment RED-strategic item.
...
| **CA notice — Broaden Proof 1 (v0.3 held-out 🔥 BURNED, DUAL-MODEL-CONSENSUS; ⚠ ERRATA 2026-07-19 — see below)** | **26 held-out (2 excluded)** | **As-frozen: 88.5% (23/26), CI [71.0%, 96.0%]. Post-errata: 96.2% (25/26), CI [81.1%, 99.3%] (Wilson, both)** | **α=1.000 (GPT/Gemini model agreement, 26/26 AGREE, 0 disagree)** | **B2 as-frozen: confident-wrong=3. B2 post-errata: confident-wrong=1 (C-18 only — the confirmed §1946.2(a) coverage gap). C-21/C-22 reclassified: dual-model-consensus was correct; frozen ground truth was erroneous (attorney errata, see `docs/ERRATA_MEMO_v0_3_20260719.docx`).** | *Status=FROZEN 2026-07-16, scored 2026-07-19 (Steps 5-7 COMPLETE, one-time burn). Distribution VALID=14/INVALID=11/UD_DEFECTIVE_PREMATURE=1 as originally frozen (12/13/1 post-errata). Held-out set now permanently burned — do not re-score. Ground-truth error rate: 2/26 = 7.7%.* | **2026-07-16 (freeze); 2026-07-19 (scored); 2026-07-19 (errata)** |
| *(future modules…)* | | | | | | |
...
| **Rule-freeze gate — dev-set regression: ✅ PASSED, 2026-07-20 10:31 PT.** | `python3 rules/validation/scorer/dev_set_monitor.py`, live run, real keys: **12/12 = 100%, `newly_failing: []`, DUAL-MODEL-CONSENSUS (α=1.000), `triggered_by_rule_change: true`.** `rules_sha256` in the output confirmed = v3's SHA (`65f1d9a4…947c7d`) exactly. Output: `rules/validation/scorer/output/ca_notice_score_2026-07-20_non-held-out.json`; trend row appended to the Direction D-1 table below. **v3 is now the fully active, gate-passed rules version.** Per Task 4 of the errata-cycle directive, this closes the directive end-to-end (score → errata → autopsy → wiring determination → proposal → ratification → new version → regression gate). |
...
**v0.3 FROZEN held-out set (freeze complete 2026-07-16; ingested 2026-07-18):**
- File: `rules/validation/scorer/FROZEN/goldenset_CA_notice_v0.3_FROZEN_20260716.xlsx` (supersedes the DRAFT file for scoring; DRAFT file retained for record)
- SHA256 (certified, per Andy's freeze memo, verified matching on ingestion): `e6dbb2fcb60de0773f9ff5594e09f74c6a6bac5670c70bd9bb76d70e2645df45`
- **Items: 26 FROZEN (scoreable), 2 EXCLUDED (C-05, C-13) — n changed from 28→26.** All FROZEN rows Held-out=TRUE (this file is exclusively a held-out set; no non-held-out partition).
- Frozen outcome distribution: NOTICE_VALID=14, NOTICE_INVALID=11, UD_DEFECTIVE_PREMATURE=1
- Reviewer: Andrew M. Cohen, 2026-07-16, item-by-item — every frozen row carries a CONFIRM verdict, Reviewed-by, and date. Ground truth is now immutable.
- Modules covered: pay-or-quit (counts, amounts, content), payee elements, notice period/day-count (SB 611), cure-or-quit (§1161(3)), unconditional quit (§1161(4)), AB 1482/§1946.2, service methods (CCP §1162)
- **Excluded (routed to open-textured queue, not scored):**
  - C-05 (utilities-as-designated-additional-rent in a §1161(2) demand) — attorney ruling: law not clear enough for immutable ground truth; companion fact pattern to v0.2's CA-NOT-09 exclusion.
  - C-13 (conditional-notice-for-nuisance) — attorney ruling: scenario too vague for immutable ground truth; the clean §1161(4) case is already covered by C-14.
- **Citation corrections made at freeze** (6 corrections; see DAILY_CHANGELOG 2026-07-16 entry and Andy's freeze memo for full detail): Levitz Furniture pincite (1411→1035); Losornio v. Motta removed from C-04 (wrong holding for the point cited); Civ. Code §1946.2(e)(7)/(e)(8) un-swapped (C-20/C-21/C-22); relocation-mechanics 15-calendar-day window corrected (C-16/C-17); CCP→Civ. Code clerical fix (C-16–C-22); §1947.12(d)(5) cross-ref added (C-22). No rule-file edits — these are golden-set authority-field corrections only; `ca_eviction_v2.json` remains untouched at vProof1.
- **Scorer bug found + fixed at ingestion (2026-07-18):** `ca_notice_scorer.py` required "Correct outcome (if corrected)" to be populated for every FROZEN row, but this file leaves it blank when ATTORNEY VERDICT=CONFIRM (i.e. "not corrected" — by the column's own name). Would have skipped all 26 items as YELLOW-INCOMPLETE. Fixed: falls back to "Drafted outcome" only when ATTORNEY VERDICT is CONFIRM/CONFIRMED; any other verdict with a blank "Correct outcome" still fails loud (YELLOW-INCOMPLETE), unchanged. No golden-set data was edited — file SHA256 unchanged, verified matching Andy's certified hash both before and after the code fix. All 26 resolved outcomes cross-checked by hand against Andy's freeze memo ruling table — exact match, including the 14/11/1 distribution. Regression tests: `test_ca_notice_scorer_outcome_fallback.py`, 15/15 pass.
- **Broaden Proof 1 Steps 5-7 — COMPLETE (scored 2026-07-19, Andy's Terminal, real keys, daytime window).** Held-out set is now permanently burned; this is a one-time result, not to be re-run. Output: `rules/validation/scorer/output/ca_notice_score_2026-07-19_held-out.json`. Rules unchanged from vProof1 at scoring time (`rules_sha256` in the output matches `cc0cfab63ae1591e2b88…` exactly); golden-set `excel_sha256` in the output matches the certified freeze hash exactly — both provenance checks pass.

**Result, as scored 2026-07-19: 23/26 = 88.5% (95% CI [71.0%, 96.0%], Wilson score interval, z=1.96).** `consensus_status: DUAL-MODEL-CONSENSUS` — both GPT and Gemini answered on all 26 items (`single_model_items=0`).

> **⚠ ERRATA (2026-07-19, same day) — corrected result: 25/26 = 96.2% (95% CI [81.1%, 99.3%], Wilson).** Attorney review triggered by the miss autopsy (below) found the frozen ground truth itself was wrong on 2 of the 3 "misses" — see `docs/ERRATA_MEMO_v0_3_20260719.docx` (signed, authoritative) / `.md` (reference copy), ERRATUM-2026-001 and -002. **Both numbers are retained per the errata memo's dual-reporting requirement (Section 4) — neither supersedes the other in the record; "as-scored" reflects ground truth as it stood at measurement, "post-errata" reflects ground truth as corrected.** Ground-truth error rate: 2/26 = 7.7%.

**B1 (coverage):** as-scored 88.5% (23/26); **post-errata 96.2% (25/26).** **B2 (confident-wrong):** as-scored 3/26 (11.5%) — CA-NOT-C-18, C-21, C-22, all `model_agreement: AGREE` (both models HIGH confidence). **Post-errata: 1/26 (3.8%) — C-18 only.** C-21 and C-22 are reclassified: the dual-model consensus answer was legally correct; the frozen ground truth was the error (attorney errata, ERRATUM-2026-001/-002). **B3 (regression):** n/a — first live score against this held-out set; no prior run to diff against. **B4 (currency):** no rule changes since the vProof1 freeze (2026-07-02); currency check not triggered.

**Krippendorff's α (GPT vs. Gemini, nominal, n=26 items, 2 raters, no missing data):** Do (observed disagreement) = 0.000, De (expected disagreement, computed from the pooled label distribution: NOTICE_INVALID=28, NOTICE_VALID=22, UD_DEFECTIVE_PREMATURE=2 across all 52 codings) = 0.540 → **α = 1 − Do/De = 1.000.** Perfect model-pair agreement on this set — consistent with the v0.2 precedent (also DUAL-MODEL-CONSENSUS, low/zero disagreement), though as before this measures inter-model agreement, not agreement with ground truth (that's B1/accuracy above).

**Comparison to v0.2:** v0.2 held-out 5/5 = 100% (n=5, directional only — too small for a meaningful CI). v0.3 held-out (n=26, first statistically meaningful held-out measurement for this module): **as-scored 88.5%; post-errata 96.2%.** Not a regression from v0.2 either way — a larger, harder, more representative sample that (post-errata) is consistent with v0.2's strong result, and additionally surfaced one genuine coverage gap (C-18) the n=5 set was too small to find.

**B2 finding — not random noise, a real coverage cluster:** all three confident-wrong items are AB 1482 just-cause/notice-period **exemption** fact patterns under Civ. Code §1946.2(e)(7) (new-construction, certificate-of-occupancy-within-15-years exemption) and §1946.2(e)(8) (separately-alienable SFH exemption, with its required prescribed-disclosure condition). In each case both models correctly computed what the *default* (non-exempt) rule would require — a stated just-cause reason (C-18) or a 60-day notice period for 1+ year tenancies (C-21, C-22) — and voided the notice for missing it, but the frozen ground truth is NOTICE_VALID because the exemption applies and the default requirement doesn't. Notably, these are the same three items where Andy's freeze review corrected the golden-set citations for exactly this (e)(7)/(e)(8) distinction (see citation-corrections note above) — the golden set's own answer key needed correcting here because the exemption boundary is genuinely easy to get wrong, and the model output shows the same boundary is not currently encoded (or not reliably triggered) in `ca_eviction_v2.json` at vProof1.

**RED — decision needed from Andy, no action taken:** per the freeze record's standing rule ("NO RULE EDITS PERMITTED... discovered gaps → next development cycle with fresh held-out set"), `ca_eviction_v2.json` has NOT been touched and no fix has been attempted. This is now a substantive rules-coverage gap for Andy to decide how to route (e.g., a Broaden Proof 2 encoding cycle for the §1946.2(e)(7)/(e)(8) exemption logic) — not a citation typo and not a scorer issue.

---

**⚠ ERRATA UPDATE (2026-07-19, same day) — the paragraphs above are the original as-scored analysis, retained for the record. Corrected analysis follows.**

Attorney review (triggered by the miss autopsy below) determined the original B2 framing was half right: C-18 is confirmed as a genuine coverage gap (§1946.2(a) attachment threshold absent from vProof1, unchanged). But **C-21 and C-22 are NOT a coverage gap** — per `docs/ERRATA_MEMO_v0_3_20260719.docx` §1: Civil Code §1946.1 governs notice-period length *independently of and in addition to* §1946.2's just-cause requirement. An AB 1482 exemption under §1946.2(e)(7)/(e)(8) removes the just-cause obligation only — it does not shorten, displace, or excuse §1946.1(b)'s 60-day notice-period requirement for a 1+-year tenancy (*Stancil v. Superior Court* (2021) 11 Cal.5th 381). Both C-21 and C-22 involve tenancies of 18 months / 2 years respectively, each served a 30-day notice — void under §1946.1(b)/Stancil regardless of AB 1482 exemption status. **The dual-model consensus answer (NOTICE_INVALID) was legally correct on both items; the frozen ground truth (NOTICE_VALID) was the error**, attributed at root to single-lens review at the 2026-07-16 freeze session (the facilitator presented C-21/C-22 solely through the AB 1482 exemption lens they were drafted to test, without an independent §1946.1 duration check — classified as an incomplete-defect-sweep failure). Full analysis, root cause, and the corrective protocol adopted for all future golden-set freezes (v0.4 forward — mandatory per-item sweep against every encoded defect class, not only the defect class the item was drafted to test): `docs/ERRATA_MEMO_v0_3_20260719.docx`.

**RED — updated status:** the §1946.2(e)(7)/(e)(8) wiring question is **RESOLVED, no rule edits warranted** — `notice_period_too_short` correctly has no `ab1482_coverage_gate` (the exemption legitimately does not reach that defect); the original "wiring gap" hypothesis in the miss autopsy is superseded. `ca_eviction_v2.json` remains untouched at vProof1 and this confirms it should. **Only C-18's §1946.2(a) attachment-threshold gap remains open and RED** — still a decision for Andy on how to route (e.g., a Broaden Proof 2 encoding cycle); still no rule edits made or attempted.

**Miss autopsy (2026-07-19, per Andy's follow-up directive):** full per-rule present/absent inspection of vProof1 in `docs/AUTOPSY_v0_3_MISSES_20260719.md`. Result is mixed, not a clean coverage gap: the §1946.2(a) 12-month attachment threshold is genuinely ABSENT (explains C-18); but §1946.2(e)(7) and (e)(8) are BOTH already present and fully encoded in vProof1 — they're just wired only to the `missing_just_cause_reason` defect, not to `notice_period_too_short` (the defect C-21/C-22 actually hit). Per the directive's own stop condition, candidate-rule drafting (Task 3) was **not executed** this cycle — a wiring/scope gap on top of an open legal question (whether the exemption should reach the notice-period defect at all) needs Andy's decision, not an autonomous rule draft. **Resolved same day:** the "open legal question" flagged here — whether the AB 1482 exemption should reach the notice-period defect — is answered NO by `docs/ERRATA_MEMO_v0_3_20260719.docx` §1: §1946.1's notice-period requirement is independent of §1946.2's just-cause exemptions. The autopsy's engineering conclusion (rules exist, correctly scoped, correctly NOT wired to `notice_period_too_short`) is confirmed correct; its factual premise (that C-21/C-22 were model errors) is superseded — they were ground-truth errors instead. Task 3 (candidate rule drafting) remains not executed, now because it is not warranted for (e)(7)/(e)(8) at all; only C-18's §1946.2(a) gap may still warrant it, pending Andy's direction.
...
| 2026-08-15 | 12/12 (100.0%) | 0 | 0.917 | PARTIAL-CONSENSUS (11/12 items dual-model) | ca_notice_score_2026-08-15_non-held-out |
| 2026-08-19 | 12/12 (100.0%) | 0 | 0.75 | PARTIAL-CONSENSUS (10/12 items dual-model) | ca_notice_score_2026-08-19_non-held-out |
