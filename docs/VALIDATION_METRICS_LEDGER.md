...
| **CA notice — Broaden Proof 1 (v0.3 held-out 🔥 BURNED, DUAL-MODEL-CONSENSUS)** | **26 held-out (2 excluded)** | **88.5% (23/26); 95% CI [71.0%, 96.0%] (Wilson)** | **α=1.000 (GPT/Gemini model agreement, 26/26 AGREE, 0 disagree)** | **B2: confident-wrong=3 (11.5%) — C-18, C-21, C-22, all AGREE-but-wrong; clustered on AB 1482 §1946.2(e)(7)/(e)(8) exemptions (see below)** | *Status=FROZEN 2026-07-16, scored 2026-07-19 (Steps 5-7 COMPLETE, one-time burn). Distribution VALID=14/INVALID=11/UD_DEFECTIVE_PREMATURE=1. Held-out set now permanently burned — do not re-score.* | **2026-07-16 (freeze); 2026-07-19 (scored)** |
| *(future modules…)* | | | | | | |
...
**Result: 23/26 = 88.5% (95% CI [71.0%, 96.0%], Wilson score interval, z=1.96).** `consensus_status: DUAL-MODEL-CONSENSUS` — both GPT and Gemini answered on all 26 items (`single_model_items=0`).

**B1 (coverage):** 88.5% (23/26), consensus-validated. **B2 (confident-wrong):** 3/26 (11.5%) — CA-NOT-C-18, C-21, C-22. All three are `model_agreement: AGREE` (GPT and Gemini reached the identical wrong outcome independently, both HIGH confidence) — not split-decisions or low-confidence guesses. **B3 (regression):** n/a — first live score against this held-out set; no prior run to diff against. **B4 (currency):** no rule changes since the vProof1 freeze (2026-07-02); currency check not triggered.
...
**Comparison to v0.2:** v0.2 held-out 5/5 = 100% (n=5, directional only — too small for a meaningful CI). v0.3 held-out 23/26 = 88.5% (n=26) is the **first statistically meaningful held-out measurement** for this module (n=26 clears the small-n caveat threshold noted when this set was frozen). Not a regression from v0.2 — a larger, harder, more representative sample surfacing a real gap the n=5 set was too small to find.
