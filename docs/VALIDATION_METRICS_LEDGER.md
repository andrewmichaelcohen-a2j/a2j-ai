...
**B1 (coverage):** v0.3 held-out — Coverage: 26/26 scored = 100% (no unknown-element omissions); Accuracy (known): 23/26 = 88.5% as-scored / 25/26 = 96.2% post-errata; Overall: same (coverage 100%). **B2:** confident-wrong = **3 as-scored → 1 post-errata** (C-18 only — the genuine coverage gap; v3 encodes the fix but it is NOT verified until the dev regression gate passes; C-21/C-22 reclassified — the models were right, the ground truth was wrong). **B3:** **regression check PENDING and REQUIRED** — v3 is the first rule change since vProof1; trigger armed 2026-07-20 07:32 PT; gate = dev 12/12 with newly_failing=0, else revert to vProof1 and report RED. **B4 (currency):** v3 embeds the 07-19 wiring determination in its own provenance; carried flag — the §1946.2(a)(2) additional-adult-tenant variant was ratified from attorney-supplied text but NOT independently verified against verbatim statute text by Cowork (flagged at proposal, flag survives ratification; recommend live-source confirmation next self-critique pass).
