...
| **CA notice — Broaden Proof 1 (v0.3 held-out, FROZEN — SCORE PENDING)** | **26 frozen held-out (2 excluded)** | **n/a (outcome test — score not yet run)** | **n/a** | **pending** | *Status=FROZEN 2026-07-16 (Andy, item-by-item, 26/28 CONFIRM, 2 EXCLUDE — see freeze record below); distribution VALID=14/INVALID=11/UD_DEFECTIVE_PREMATURE=1; ingested + scorer bug fixed 2026-07-18 (see DAILY_CHANGELOG); Steps 5-7 (live dual-model score, daytime window) NOT YET RUN — pending Andy, real API keys* | **2026-07-16 (freeze); 2026-07-18 (ingested)** |
| *(future modules…)* | | | | | | |
...
- **Next step (Broaden Proof 1 Steps 5-7, NOT YET RUN):** score ONCE, dual-model consensus, against rules vProof1 (`cc0cfab63ae1591e2b88…`, unchanged), in the DAYTIME window (overnight lane still recovering from the environment RED — do not risk this precious one-time run on it). Requires real OPENAI_API_KEY/GOOGLE_API_KEY — this sandbox only has placeholder keys. Command: `python3 rules/validation/scorer/ca_notice_scorer.py --golden rules/validation/scorer/FROZEN/goldenset_CA_notice_v0.3_FROZEN_20260716.xlsx --held-out-only` (no `--dry-run`). Report held-out rate, 95% CI (binomial, e.g. Wilson score — not currently auto-computed by the scorer), B1-B4, and Krippendorff's α (computed by hand from the run's agree/disagree counts per the v0.2 precedent above — n=26 clears more of the small-n unreliability than n=5, though still modest).
